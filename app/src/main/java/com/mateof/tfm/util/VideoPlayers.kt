...
/**
 * Hands video playback off to an external app when the user picked one in
 * settings. Streams the URL directly — external players fetch it themselves,
 * so nothing is downloaded to the device.
 */
@Singleton
class VideoPlayers @Inject constructor(
    ...
    /**
     * Returns true when the video was handed off; false means the caller should
     * fall back to the built-in player (also when the chosen app is gone).
     */
